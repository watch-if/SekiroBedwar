...
| **忍具商店** (`SekiroShopManager`) | 插件自管 GUI 商店：接管商店主页右下「下一页」按钮位为入口，全部插件商品（攻速/巴之雷/长矛/纸人/漂流纸人/雾璃鸦/佛珠/盾牌）的判定、扣费、发放都在本插件内完成，动态价格实时渲染 |
...
| **巴之雷** (`LightningManager`) | 忍具商店两级购买（不可跳级）：三连击接跳斩落雷（L1）、忠诚三叉戟衔接（L2）、雷反；落雷消耗纸人 |
...
全部插件商品（剑攻速 / 巴之雷 / 长矛 / 纸人 / 漂流纸人 / 雾璃鸦 / 佛珠 / 盾牌）集中在自管 GUI。入口**接管 BedWars 商店主页右下的「下一页」按钮位**：打开商店后该槽位被替换为「只狼忍具」入口（LOWEST 优先级拦截点击，先于 BedWars 的翻页处理），点击即开 GUI，不占用 shop.yml 商店条目（分类子页的原生翻页不受影响；`shop.page-forward-takeover: false` 可切换为 shop.yml 入口物品模式）。门槛、货币校验、扣费、发放在 SekiroBedwar 插件内完成，纸人动态价 / 佛珠递增价等如实显示与校验；剑攻速与巴之雷各为**单个按钮**——显示下一级效果与价格，购买后按钮自动更新到再下一级（逐级不可跳级）。购买失败（货币不足 / 已满级 / 达上限）只提示不扣费。
...
忍具商店购买（默认 20 金）获得一只绑定的末影之眼，每人每局限购 1 只（按购买次数判定，不看背包持有）。右键即激活（拦截原版投掷，眼不飞出；消耗 1 雾璃鸦 + 2 纸人，成功/失败均有音效反馈）：末影之眼悬停在头顶 2 秒，期间受到的第一次玩家来源伤害（近战或弓箭）被完全取消（不扣血），并把玩家传送到攻击方身后的安全方块，随后雾璃鸦破碎；2 秒内未受击则破碎、免伤效果随之失效。绑定雾璃鸦不可丢弃、不放容器、死亡清除。激活消耗或死亡失去后 10 秒（`refill-delay-seconds`）自动向背包补发一只（带拾取音效，仍受持有上限约束）。
...
| `shop` | 忍具商店：入口接管开关（page-forward-takeover）、入口物品（材质/名称/lore）、GUI 标题；全部插件商品在 GUI 内购买 |
