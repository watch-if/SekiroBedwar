...
| 模块 | 说明 |
| --- | --- |
| **秘传·飞渡浮舟** (`FeiduFuzhou`) | 七段节奏连击（7.3/10/5.7/5.3/5.7/16 tick ±0.2）：第 6 击有效命中额外 −10 架势；全连达成奖 2 纸人 + 5 架势 + 1 HP |
...
| **秘传·一心七连** (`IsshinSevenStrike`) | 七段节奏连击（7/5/5/6/8/10 tick ±0.2），第 3 段起有效命中逐段叠加架势增伤；**第 7 击必须是危攻击**；全连达成奖 2 纸人 + 4 架势 |
...
| **玩法指南书** (`WelcomeManager`) | 进入服务器或离开对局回到大厅时发放 10 页成书（玩法定位与心法介绍）；进入对局自动收回 |
...
四式秘传**全局生效、无需购买**，在 ACTIVE 决斗内按节奏打出近战连击即可触发。共同规则：

- **节奏判定**：相邻两击的**命时间隔**须落在目标值 ±0.2 tick 内（被完美弹反的命中仍算打出的一击，连段继续；任何一击脱拍则打磨音提示、本击作为新的一式重连）。
- **空手换刀式**（苇名 / 龙闪）：先持续空手再快捷栏换持近战武器进入「已武装」，武装后**起手动作须在 1 tick 内衔接**（换刀即拔刀）；两式空手时长窗口互斥——0.5~1s 归苇名，≥1s 归龙闪。
- **音效语言**：成功接段 = 铁砧落地声，脱拍 = 铁砧打磨声（一次连续脱拍只播一次打磨）。
...
七段近战连击，相邻命时间隔依次 **7.3 / 10 / 5.7 / 5.3 / 5.7 / 16 tick**（各 ±0.2）。
...
空手蓄势 0.5~1 秒后换持近战武器，起手第一击（1 tick 内衔接）命中起计时，**第二击命时间隔 4 tick ±0.5**。
...
七段近战连击，相邻命时间隔依次 **7 / 5 / 5 / 6 / 8 / 10 tick**（各 ±0.2）。

- **第 3 段起**每段成功播落地音，且该段为有效攻击时受击方被**逐段递增**额外扣架势：第 3 段 −3、第 4 段 −6、第 5 段 −9、第 6 段 −12（按已叠有效段数 ×3；被弹反的段不叠加但连段继续）。
...
| `mystery` | 秘传框架：`arm-connect-ticks` 换刀衔接窗；四式各自一段（`fei-du-fu-zhou` / `yamedo-cross-slash` / `long-shan` / `isshin-seven-strike`：间隔序列、容差、加成与奖励数值） |
