...
| 模块 | 说明 |
| --- | --- |
| **秘传·飞渡浮舟** (`FeiduFuzhou`) | 七段节奏连击（7.3/10/5.7/5.3/5.7/16 tick ±0.2）：第 3 击起落地音 + 1s 防击退（刷新）；第 6 击有效命中额外 −10 架势；全连达成奖 2 纸人 + 5 架势 + 1 HP |
...
| **秘传·一心七连** (`IsshinSevenStrike`) | 七段节奏连击（7/5/5/6/8/10 tick ±0.2），第 3 击起落地音 + 1s 防击退（刷新）、有效命中逐段叠加架势增伤；**第 7 击必须是危攻击**；全连达成奖 2 纸人 + 4 架势 |
...
- **音效语言**：成功接段 = 铁砧落地声，脱拍 = 铁砧打磨声（一次连续脱拍只播一次打磨）。飞渡浮舟与一心七连均自**第三击**起每段成功播落地音。
